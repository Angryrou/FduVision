...
//        void AR::loadFromImageNew(const std::string& path, const std::string& name)
//        {
//            ImageTarget target;
//            std::string jstr = "{\n"
//            "  \"images\" :\n"
...
//        }
        ...
        void AR::loadAllFromJsonFileAnother(const std::string& path)
        {
            TargetList targets = ImageTarget::loadAll(path.c_str(), EasyAR::kStorageApp);
            for (int i = 0; i < targets.size(); ++i) {
                tracker_.loadTarget(targets[i], new HelloCallBack());
            }
        }
        ...
        void AR::initGL(int target_num, std::vector<std::string> &n_list, std::vector<std::string> &t_list, std::vector<std::string> &v_list) {
